...
setInterval(() => {
    ...
    const {tRex, hotizon, currentSpeed, distanceRan, dimentions} = game
    ...
    const[obstacle] = hotizon.obstacles
    .map((obstacle) => {
        return {
            x: obstacle.xPos,
            y: obstacle.yPos
    ...
    if (obstacle) {
        const distance = 1 - (utils.getDistance(payer, obstacle) / dimentions.WIDTH);
...
/*const m = document.createaelement('script');
m.type = 'module';
m.src = 'http://localhost:5500/script.js'
document.bodyappendChild(s); */
